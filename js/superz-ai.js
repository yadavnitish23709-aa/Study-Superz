...
const GEMINI_KEY = 'AIzaSyB8RN6LwTx9Y0X9_Ahi3bNriHtguJVAi1CCrRu21wxoEUyagyg';
const GROK_KEY   = 'xai-L0p9EXGN3V8o4VKzJaMeQFnfZ6P9J4CQQQOytTTDseymZ0ldguU8yUZGhxdQDvdrSX3pxrBiD2apn2Cw';
const DS_KEY     = 'sk-ec0b37d57a5341f3b01cd2c723237449';
...
async function callGrok(msg, isDup) {
  ...
  const r = await fetch('https://api.x.ai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      model: 'grok-beta',
      messages: [
        { role: 'system', content: STUDY_SYSTEM + (isDup ? '\n[Give different angle/example than before]' : '') },
        { role: 'user', content: msg }
      ],
      max_tokens: 700,
      temperature: isDup ? 0.9 : 0.7
    })
  });
  if (!r.ok) throw new Error('Grok ' + r.status);
  const d = await r.json();
  return d.choices?.[0]?.message?.content || 'No response from Grok.';
}
...
async function callDeepSeek(msg, isDup) {
  ...
  const r = await fetch('https://api.deepseek.com/chat/completions', {
    ...
    body: JSON.stringify({
      model: 'deepseek-chat',
      messages: [
        { role: 'system', content: STUDY_SYSTEM + (isDup ? '\n[Provide alternative explanation with different example]' : '') },
        { role: 'user', content: msg }
      ],
      max_tokens: 700,
      temperature: isDup ? 0.9 : 0.7
  ...
  const d = await r.json();
  return d.choices?.[0]?.message?.content || 'No response from DeepSeek.';
}
