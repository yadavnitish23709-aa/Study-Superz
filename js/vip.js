      ...
      try {
        ...
        if (model === 'grok') {
          const r = await fetch('https://api.x.ai/v1/chat/completions', {
            ...
            body: JSON.stringify({ model: 'grok-beta', messages: [{ role: 'system', content: VIP_SYSTEM }, { role: 'user', content: msg }], max_tokens: 700, temperature: 0.85 })
          });
          ...
          const r = await fetch('https://api.deepseek.com/chat/completions', {
            ...
            body: JSON.stringify({ model: 'deepseek-chat', messages: [{ role: 'system', content: VIP_SYSTEM }, { role: 'user', content: msg }], max_tokens: 700, temperature: 0.85 })
          });
          const d = await r.json(); reply = d.choices?.[0]?.message?.content || 'No response.';
        } else {
